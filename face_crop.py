...
class FaceCrop(object):
    ...
    def __init__(self, current_image, to_be_cropped):
    ...
    def detect(self, image, message_id):
        ...
        # Multiple faces
        if len(result) > 1:
            prompt = self.prompt_plot(self, result)
            # Update dict to preserve for after user choice
            if message_id not in self.to_be_cropped.keys():
                images_status = []
                # Image only switched to True when user chooses in prompt
                for image in result: images_status.append((image, False))
                self.to_be_cropped[message_id] = images_status
            return prompt

        elif len(result) == 1:
            result_after_cropping = find_faces.find_face(result)
